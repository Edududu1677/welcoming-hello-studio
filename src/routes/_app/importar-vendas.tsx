...
function ImportarVendas() {
  ...
  const { data: batches } = useQuery({
    ...
    queryFn: async () => (await supabase.from("import_batches").select("id, arquivo_nome, status, total_registros, total_sucesso, total_erros, total_duplicados, created_at").eq("tipo", "vendas").order("created_at", { ascending: false }).limit(20)).data ?? [],
  });
  ...
  async function runImport() {
    ...
    const { data: batchRow } = await supabase.from("import_batches").insert({
      tipo: "vendas", subtipo: "diario", created_by: user?.id, status: "processando", total_registros: rows.length,
    }).select("id").single();
    ...
    for (const r of rows) {
      try {
        ...
        const dataVenda = parseDate(mapping.data ? r[mapping.data] : null);
        ...
        // Duplicate check
        if (codVenda) {
          const { data: dup } = await supabase.from("sales").select("id").eq("codigo_venda", codVenda).eq("codigo_barras", cb).eq("quantidade", qtd).maybeSingle();
          if (dup) { duplicados++; continue; }
        }

        // Find product
        const { data: prod } = await supabase.from("products").select("id, custo_medio").eq("codigo_barras", cb).maybeSingle();
        let productId: string | null = prod?.id ?? null;
        let custoUnit = prod ? Number(prod.custo_medio) : 0;
        if (!prod) {
          // Create pending product
          const { data: novo } = await supabase.from("products").insert({
        ...
        await supabase.from("sales").insert({
          product_id: productId, codigo_barras: cb, quantidade: qtd,
          preco_unitario: preco, valor_total: valorTot,
          custo_unitario: custoUnit, lucro_bruto: lucroBruto,
          data_venda: dataVenda, codigo_venda: codVenda, forma_pagamento: forma,
    ...
    if (batchId) await supabase.from("import_batches").update({
      ...
      total_sucesso: ok, total_erros: erros.length, total_duplicados: duplicados, completed_at: new Date().toISOString(),
    }).eq("id", batchId);
  ...
  async function undoBatch(id: string) {
    ...
    const { data: sales } = await supabase.from("sales").select("id, product_id, quantidade").eq("import_batch_id", id);
    for (const s of sales ?? []) {
      if (s.product_id) {
        await supabase.rpc("apply_stock_movement" as any, {
          _product_id: s.product_id, _tipo: "cancelamento_venda", _quantidade: Math.abs(Number(s.quantidade)),
          _custo: null, _motivo: "Reversão de importação", _documento_ref: id,
        });
      }
    }
    await supabase.from("sales").delete().eq("import_batch_id", id);
    await supabase.from("import_batches").update({ status: "revertido" }).eq("id", id);
  ...
  return (
    <div className="space-y-4 max-w-5xl">
      ...
      <Card>
        ...
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            <TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Total</TableHead><TableHead className="text-right">Sucesso</TableHead><TableHead className="text-right">Duplic.</TableHead><TableHead></TableHead></TableRow></TableHeader>
            <TableBody>
              {(!batches || batches.length === 0) && <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">Nenhuma importação registrada</TableCell></TableRow>}
              {batches?.map((b: any) => (
                <TableRow key={b.id}>
                  ...
                  <TableCell className="text-right">{num(b.total_registros)}</TableCell>
                  <TableCell className="text-right">{num(b.total_sucesso)}</TableCell>
                  <TableCell className="text-right">{num(b.total_duplicados)}</TableCell>
